...
async def _preload(queue: str, n: int, *, persistent: bool = True) -> None:
    ...
    b = AsyncBroker(RabbitConfig(safety=_CHAOS_SAFETY, connection=ConnectionConfig.from_url(URL),
...
async def scenario_restart_during_consume() -> tuple[bool, str]:
    ...
    broker = AsyncBroker(RabbitConfig(safety=_CHAOS_SAFETY, connection=ConnectionConfig.from_url(URL)))
...
def _run_sync_consumer(queue: str, n: int, processed: set[str], dups: list[int],
    ...
    """
    broker = SyncBroker(RabbitConfig(safety=_CHAOS_SAFETY, connection=ConnectionConfig.from_url(URL)))
...
def _run_sync_retry_consumer(queue: str, retry_cfg: RetryConfig, attempts: list[int],
                             ready: threading.Event) -> None:
    broker = SyncBroker(RabbitConfig(safety=_CHAOS_SAFETY, connection=ConnectionConfig.from_url(URL), retry=retry_cfg))
...
async def scenario_sync_retry_to_dlq() -> tuple[bool, str]:
    ...
    inspect_broker = AsyncBroker(RabbitConfig(safety=_CHAOS_SAFETY, connection=ConnectionConfig.from_url(URL)))
