...
- [ ] **Bulk publishing goes through `publish_many` / `iter_publish`**, never a hand-rolled loop that ignores per-item outcomes. Treat `UNKNOWN` items as "reconcile", `NOT_SENT` as "safe to resubmit", and never replay a whole batch. Bound every call (`BulkPublishOptions`: in-flight, bytes, admission/overall timeouts). See [Bulk Operations](../bulk-operations.md).
- [ ] **Deferred / batch-commit consumers use `ack_many` / `nack_many`** after the transaction commits, for the exact successful subset. Never `basic_ack(multiple=True)` from application code — use `CoalescingAcker` if you need coalescing; the legacy `BatchAcker` cumulative mode now requires an explicit ownership attestation.
- [ ] **Critical flows: run `broker.preflight("critical", management_client=...)`** at deploy time and fail the rollout on `FAILED`; log `UNVERIFIED` checks instead of assuming green. Apply the `policy_templates()` output (`dead-letter-strategy: at-least-once`, `overflow: reject-publish`, `delivery-limit`) through your topology owner.
...
- [ ] Scrape the emitted metrics (ack/nack/reject/retry/dead-letter counters, handler duration/errors). Don't build alerts on a metric name that isn't actually emitted — see [`docs/observability.md`](../observability.md) for the exact, current list (`publish_total` and `publish_failures_total` were removed in 0.14 — they were never emitted).
...
- [ ] Alert on `rabbitkit_bulk_publish_items_total{status="unknown"}` (reconcile backlog), rising `rabbitkit_retry_handoff_failures_total` / `rabbitkit_retry_handoff_paused == 1` (delay-queue topology unreachable — nothing is progressing), and `rabbitkit_settlement_items_total{status="stale"}` (deferred acks outliving their channel). The lifecycle gauges (`broker_connected`, `consumer_active`, `in_flight_messages`, `publish_confirm_latency_seconds`) are emitted since 0.12.
- [ ] Alert on rising `rabbitkit_messages_redelivered_total` (handlers dying/timing out before acking — crash loops, heartbeat kills), `rabbitkit_reconnects_total` (flapping broker/network — **sync broker only**; async does not emit it, see [observability](../observability.md#async-does-not-emit-reconnects_total)), and `rabbitkit_channel_rebuilds_total` (channel churn from reconnects/topology drift — the signal to use on async) — all invisible in the success/error counters.
