...
CONFIG = RabbitConfig(
    connection=ConnectionConfig(
        ...
        blocked_connection_timeout=300.0,   # give up if the broker holds us blocked (mem/disk alarm)
        reconnect_backoff_base=1.0, reconnect_backoff_max=30.0,   # exponential reconnect backoff
    ...
    security=SecurityConfig(
        mechanism="PLAIN",
        ssl=SSLConfig(enabled=False,        # set True + certs for AMQPS (port 5671) in prod
...
MIDDLEWARES = [
    ...
    DeduplicationMiddleware(redis, DeduplicationConfig(            # Redis fast-path idempotency
        key_prefix="full:dedup", ttl=86400, key_source="message_id",
        fallback_on_redis_error=True)),                           # Redis down → process anyway (DB is truth)
