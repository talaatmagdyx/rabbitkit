...
| P6 | `MetricsMiddleware` + Prometheus on `:9102` | Publish outcomes by real status, retry/DLQ counters, reconnect + channel-churn counters — [the metric reference](../../docs/observability.md) |
| P7 | Liveness ≠ readiness on `:8082` | A broker outage must flip **readiness** (stop traffic), never **liveness** (don't restart the fleet mid-outage) — served from a plain stdlib HTTP thread; rabbitkit's health functions are safe to call from any thread |
