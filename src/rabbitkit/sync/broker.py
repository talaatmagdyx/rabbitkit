...
from rabbitkit.core.bulk_runner import iter_publish_sync
...
from rabbitkit.core.settlement import SettlementItem, SettlementReport, settle_many_sync
...
class SyncBroker:
    ...
    def iter_publish(
        ...
        """Publish a (possibly unbounded) iterable, yielding one
        :class:`~rabbitkit.core.bulk.BulkPublishItem` per input as it settles.

        Every item goes through the SAME path as :meth:`publish` — publish
        middlewares, flow control, size limit, transport — so single and
        bulk publishing share every safeguard. The sync transport confirms
        one publish at a time (pika's BlockingConnection cannot pipeline
        confirms), so this is sequential; ``options.max_in_flight`` has no
        effect here. ``options.overall_timeout`` does: past the deadline
        remaining inputs are yielded ``NOT_SENT`` without touching the wire.

        Memory is bounded by the caller: results are yielded, not collected.
        """
        if self._transport is None:
            raise BrokerNotStartedError("Broker not started. Call start() first.")
    ...
    def publish_many(
        ...
        """Publish a bounded collection and return input-ordered per-item outcomes.
        ...
        :meth:`iter_publish` for streams. See
        :class:`~rabbitkit.core.types.BulkPublishStatus` for what each state
        means and what to do about it — in particular ``UNKNOWN`` items may
        have reached the broker and must be reconciled, not blindly replayed.
        """
        opts = options or BulkPublishOptions()
        ...
        items = list(self.iter_publish(envelopes, opts))
    ...
    def ack_many(self, messages: Iterable[RabbitMessage], *, fail_fast: bool = True) -> SettlementReport:
        ...
        acknowledgement for a consumer ack. Messages must come from THIS
        broker (MANUAL ack policy, or a handler that deferred settlement).
        """
        return settle_many_sync(
            messages,
